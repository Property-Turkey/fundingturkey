<?php 
    ...
    define("APP_FOLDER",  $_SERVER['SERVER_NAME'] == 'localhost' ? '/fundingturkey' : '' );
    define("PAGES", [
        'home' => 'Pages/home.php',
        'about-us' => 'Pages/aboutus.php',
        'our-funds' => 'Pages/funds.php',
        'frequently-asked-questions' => 'Pages/faq.php',
        'contact-us' => 'Pages/contactus.php',
        'news' => 'Pages/news.php',
    ...
    function getCurrentPage()
    {
        ...
        $url_without_query = explode( '?', $_SERVER['REQUEST_URI'] ) [0];
        $url = array_values ( array_filter ( explode('/', str_replace( APP_FOLDER, '', $url_without_query ) ) ) );

        if(empty($url)){ return PAGES['home']; }

        $page_key = isset($url) ? $url[0] : 'home';

        if( in_array( $url[0], LANGUAGES ) ){
            $page_key = count( $url ) > 1 ? $url[1] : 'home';
        }
        $currPage = $page_key;
